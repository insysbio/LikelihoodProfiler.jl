using DiffEqBase, OrdinaryDiffEq, DiffEqMonteCarlo, ParameterizedFunctions, DiffEqParamEstim,
CSV, DataStructures, Distributions, StatsFuns, Sundials, IterableTables, DataFrames

#using PrintLog
#@printlog "DREAM6_identification.log"

# Input data
exp_data = CSV.read("Z:/DREAM6/input/exp_data.csv", types=Dict("t"=>Float64), nullable=false)
u_data = CSV.read("Z:/DREAM6/input/u_data.csv", types=Dict("values"=>Float64), nullable=false)
p_data = CSV.read("Z:/DREAM6/input/p_data.csv", types=Dict("values"=>Float64), nullable=false)
...
obj = build_loss_objective(ODEProblem(Model1, u0, (0., 20.), p0), AutoTsit5(Rosenbrock23()), ofv_loss,
verbose=false, maxiter=1e6, saveat=exp_data[:t])

# identifiability level
obj0 = obj(p0) + chisqinvcdf(1,0.95)

# Scale types
direct_scale = fill(false,length(p0))
log_scale = fill(true,length(p0))
mixed_scale = fill(false,length(p0))
mixed_scale[[15,17,19,21,23,25,27,29]] = true

# Bound types
bounds_id = fill([1e-9,1e9],length(p0))
[bounds_id[i] = [1e-9,9.] for i in [15,17,19,21,23,25,27,29]]

bounds_params_open = fill([-Inf,Inf],length(p0))
bounds_params_closed = fill([1e-12,1e12],length(p0))
bounds_params_half_open = fill([1e-12,Inf],length(p0))
#=
using PrintLog
