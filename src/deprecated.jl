profile(plprob::ProfileLikelihoodProblem{ParameterProfile}, method::AbstractProfilerMethod; 
...
Base.depwarn("`profile` is deprecated. Please use `solve` instead.", :profile)
...
PLProblem(optprob::OptimizationProblem, optpars::AbstractVector{<:Real},
  ...
  conf_level::Float64 = 0.95, df::Int = 1, threshold::Real = chi2_quantile(conf_level, df)) = 
    ProfileLikelihoodProblem(optprob, optpars, profile_range=profile_range; 
      conf_level=conf_level, df=df, threshold=threshold)

Base.depwarn("`PLProblem` is deprecated. Please use `ProfileLikelihoodProblem` instead.", :PLProblem)
