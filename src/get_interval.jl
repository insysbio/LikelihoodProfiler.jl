...
## Arguments
- `theta_init`: starting values of parameter vector ``\\theta``. The starting
values is not necessary to be the optimum values for `loss_func` but it the value
of `loss_func` must be lower than `loss_crit`.
...
`:direct, :log, :logit`. This option can make optimization much more faster,
especially for wide `theta_bounds`. The default value is `:direct` (no transformation)
for all components.
- `theta_bounds`: vector of bounds for each component in format `(left_border, right_border)`.
This bounds define the ranges for possible parameter values. The defaults are the non-limited
values taking into account the `scale`, i.e. ``(0., Inf)`` for `:log` scale.
- `scan_bounds`: vector of scan bound for `theta_num` component. It must be
within the `theta_bounds` for the scanned component. The defaults are ``(-9., 9.)``
for transformed values, i.e. ``(1e-9, 1e9)`` for `:log` scale.
...
algorithms form NLOPT pack were tested. The methods: `:LN_NELDERMEAD, :LN_COBYLA, :LN_PRAXIS`
show good results. Methods: `:LN_BOBYQA, :LN_SBPLX, :LN_NEWUOA` is not recommended.
