...

{
    ...
    "category": "method",
    "text": "get_interval(\n    theta_init::Vector{Float64},\n    theta_num::Int,\n    loss_func::Function,\n    method::Symbol;\n\n    loss_crit::Float64 = 0.0,\n    scale::Vector{Symbol} = fill(:direct, length(theta_init)),\n    theta_bounds::Vector{Tuple{Float64,Float64}} = unscaling.(\n        fill((-Inf, Inf), length(theta_init)),\n        scale\n        ),\n    scan_bounds::Tuple{Float64,Float64} = unscaling.(\n        (-9.0, 9.0),\n        scale[theta_num]\n        ),\n    scan_tol::Float64 = 1e-3,\n    loss_tol::Float64 = 1e-3,\n    local_alg::Symbol = :LN_NELDERMEAD,\n    kwargs...\n    )\n\nComputes confidence interval for single component theta_num of parameter vector and loss_func according to loss_crit level.\n\nReturn\n\nParamInterval structure storing all input data and estimated confidence interval.\n\nArguments\n\ntheta_init: starting values of parameter vector theta. The starting\n\nvalues is not necessary to be the optimum values for loss_func but it the value of loss_func must be lower than loss_crit.\n\ntheta_num: number n of vector component to compute confidence interval theta^n.\nloss_func: loss function Lambdaleft(thetaright) the profile of\n\nwhich is analyzed. Usually we use log-likelihood for profile analysis in form Lambda( theta ) = - 2 lnleft( L(theta) right).\n\nmethod: computational method to evaluate interval endpoint. Currently the\n\nfollowing methods are implemented: :CICO_ONE_PASS, :LIN_EXTRAPOL, :QUADR_EXTRAPOL.\n\nKeyword arguments\n\nloss_crit: critical level of loss function. The endpoint of CI for selected\n\nparameter is the value at which profile likelihood meets the value of loss_crit.\n\nscale: vector of scale transformations for each component. Possible values:\n\n:direct, :log, :logit. This option can make optimization much more faster, especially for wide theta_bounds. The default value is :direct (no transformation) for all components.\n\ntheta_bounds: vector of bounds for each component in format (left_border, right_border).\n\nThis bounds define the ranges for possible parameter values. The defaults are the non-limited values taking into account the scale, i.e. (0 Inf) for :log scale.\n\nscan_bounds: vector of scan bound for theta_num component. It must be\n\nwithin the theta_bounds for the scanned component. The defaults are (-9 9) for transformed values, i.e. (1e-9 1e9) for :log scale.\n\nscan_tol: Absolute tolerance of scanned component (stop criterion).\nloss_tol: Absolute tolerance of loss_func at loss_crit (stop criterion).\n\nRestriction. Currently is not effective for :CICO_ONE_PASS methods because of limitation in LN_AUGLAG interface.\n\nlocal_alg: algorithm of optimization. Currently the local derivation free\n\nalgorithms form NLOPT pack were tested. The methods: :LN_NELDERMEAD, :LN_COBYLA, :LN_PRAXIS show good results. Methods: :LN_BOBYQA, :LN_SBPLX, :LN_NEWUOA is not recommended.\n\nkwargs...: the additional keyword arguments passed to get_right_endpoint\n\nfor specific method.\n\n\n\n\n\n"
},
